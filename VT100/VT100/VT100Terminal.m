        ...
        size_t *rmlen)
{
    ...
    copyString(&result.u.string, datap, *rmlen, NSASCIIStringEncoding);
        ...
        size_t *rmlen, NSStringEncoding encoding)
{
    ...
    *rmlen = 0;
    ...
    if (result.type != VT100_WAIT) {
        ...
        copyString(&result.u.string, datap, *rmlen, encoding);
        if (result.u.string==nil) {
            int i;
...
void copyString(unsigned char ** target, const unsigned char * res, size_t len, NSStringEncoding encoding) {
    
    if (*target != NULL) {
        free(*target);
    }
    size_t utf8len = len;
    const char * utf8Res = res;
    if (encoding != NSUTF8StringEncoding) {
        NSString *content = [[NSString alloc] initWithBytes:res length:len encoding:encoding];
        utf8Res = content.UTF8String;
        utf8len = strlen(utf8Res);
    }
    *target = calloc(utf8len, sizeof(unsigned char));
    memcpy(*target, utf8Res, utf8len);
}
